...
int RGWRadosUser::list_buckets(const string& marker, const string& end_marker,
			       uint64_t max, bool need_stats, RGWBucketList &buckets, const jspan* const parent_span)
{
...
int RGWRadosStore::get_bucket(RGWUser* u, const rgw_bucket& b, std::unique_ptr<RGWBucket>* bucket, const jspan* const parent_span)
{
  int ret;
  RGWBucket* bp;
...
int RGWRadosStore::get_bucket(RGWUser* u, const std::string& tenant, const std::string&name, std::unique_ptr<RGWBucket>* bucket, const jspan* const parent_span)
{
  rgw_bucket b;
...
static int rgw_op_get_bucket_policy_from_attr(RGWRadosStore *store,
				       ...
				       RGWAccessControlPolicy *policy, const jspan* const parent_span = nullptr)
{
  [[maybe_unused]] const auto span = jaeger_tracing::child_span(__PRETTY_FUNCTION__, parent_span);
...
int RGWRadosStore::create_bucket(RGWUser& u, const rgw_bucket& b,
				 ...
				 bool *existed,
				 req_info& req_info,
				 std::unique_ptr<RGWBucket>* bucket_out, const jspan* const parent_span)
{
  [[maybe_unused]] const auto span = jaeger_tracing::child_span(__PRETTY_FUNCTION__, parent_span);
  int ret;
  bufferlist in_data;
