...
protected:
  int init_permissions(RGWOp* op, const jspan* const parent_span = nullptr) override {
    return 0;
  }

  int read_permissions(RGWOp* op, const jspan* const parent_span = nullptr) override {
    return 0;
  }
...
void RGWPSCreateNotif_ObjStore::execute(const jspan* const parent_span)
{
...
void RGWPSDeleteNotif_ObjStore::execute(const jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
void RGWPSListNotifs_ObjStore::execute(const jspan* const parent_span)
{
  ups.emplace(store, s->owner.get_id());
  auto b = ups->get_bucket(bucket_info.bucket);
