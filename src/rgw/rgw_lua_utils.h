...
void create_metatable(lua_State* L, void* upvalue, bool toplevel=false)
{
  if (upvalue) {
    // create table
    lua_newtable(L);
...
template<typename MetaTable>
void create_metatable(lua_State* L, std::unique_ptr<typename MetaTable::Type>& ptr, bool toplevel=false)
{
  if (ptr) {
    create_metatable<MetaTable>(L, reinterpret_cast<void*>(ptr.get()), toplevel);
  } else {
    lua_pushnil(L);
  }
}
