...
public:
  ...
  void send_response_begin(bool has_buckets, const jspan* const parent_span = nullptr) override {
    sent_data = true;
  }

  void send_response_data(rgw::sal::RGWBucketList& buckets, const jspan* const parent_span = nullptr) override {
    if (!sent_data)
      return;
  ...
  void send_response_end(const jspan* const parent_span = nullptr) override {
    // do nothing
  }
...
public:
  ...
  int read_permissions(RGWOp* op_obj, const jspan* const parent_span = nullptr) override {
...
public:
  ...
  int header_init() override {
    ...
    /* XXX required in RGWOp::execute(const jspan* const parent_span) */
  ...
  int get_data(buffer::list& _bl, const jspan* const parent_span = nullptr) override {
...
public:
  ...
  int send_response_data(ceph::buffer::list& bl, off_t bl_off,
                         off_t bl_len, const jspan* const parent_span = nullptr) override {
    size_t bytes;
  ...
  int send_response_data_error(const jspan* const parent_span = nullptr) override {
...
public:
  ...
  int send_response_data(ceph::buffer::list& _bl, off_t s_off,
                         off_t e_off, const jspan* const parent_span = nullptr) override {
    /* NOP */
    /* XXX save attrs? */
    return 0;
  }
...
public:
  ...
  virtual int get_params(const jspan* const parent_span = nullptr) {
    return 0;
  }
...
public:
  ...
  int get_params(const jspan* const parent_span = nullptr) override { return 0; }
  bool only_bucket() override { return false; }
  void send_response(const jspan* const parent_span = nullptr) override {
