...
void RGWOp_MDLog_List::execute(const jspan* const parent_span) {
...
void RGWOp_MDLog_List::send_response(const jspan* const parent_span) {
...
void RGWOp_MDLog_Info::execute(const jspan* const parent_span) {
  num_objects = s->cct->_conf->rgw_md_log_max_shards;
  period = store->svc()->mdlog->read_oldest_log_period();
  op_ret = period.get_error();
}

void RGWOp_MDLog_Info::send_response(const jspan* const parent_span) {
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s);
...
void RGWOp_MDLog_ShardInfo::execute(const jspan* const parent_span) {
  string period = s->info.args.get("period");
  string shard = s->info.args.get("id");
  string err;
...
void RGWOp_MDLog_ShardInfo::send_response(const jspan* const parent_span) {
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s);
...
void RGWOp_MDLog_Delete::execute(const jspan* const parent_span) {
  string   marker = s->info.args.get("marker"),
...
void RGWOp_MDLog_Lock::execute(const jspan* const parent_span) {
...
void RGWOp_MDLog_Unlock::execute(const jspan* const parent_span) {
...
void RGWOp_MDLog_Notify::execute(const jspan* const parent_span) {
...
void RGWOp_BILog_List::execute(const jspan* const parent_span) {
...
void RGWOp_BILog_List::send_response(const jspan* const parent_span) {
  if (sent_header)
    return;
...
void RGWOp_BILog_Info::execute(const jspan* const parent_span) {
  string tenant_name = s->info.args.get("tenant"),
         bucket_name = s->info.args.get("bucket"),
         bucket_instance = s->info.args.get("bucket-instance");
  RGWBucketInfo bucket_info;
...
void RGWOp_BILog_Info::send_response(const jspan* const parent_span) {
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s);
...
void RGWOp_BILog_Delete::execute(const jspan* const parent_span) {
  string tenant_name = s->info.args.get("tenant"),
         bucket_name = s->info.args.get("bucket"),
...
void RGWOp_DATALog_List::execute(const jspan* const parent_span) {
  string   shard = s->info.args.get("id");
...
void RGWOp_DATALog_List::send_response(const jspan* const parent_span) {
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s);
...
void RGWOp_DATALog_Info::execute(const jspan* const parent_span) {
  num_objects = s->cct->_conf->rgw_data_log_num_shards;
  op_ret = 0;
}

void RGWOp_DATALog_Info::send_response(const jspan* const parent_span) {
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s);
...
void RGWOp_DATALog_ShardInfo::execute(const jspan* const parent_span) {
  string shard = s->info.args.get("id");
  string err;
...
void RGWOp_DATALog_ShardInfo::send_response(const jspan* const parent_span) {
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s);
...
void RGWOp_DATALog_Notify::execute(const jspan* const parent_span) {
...
void RGWOp_DATALog_Delete::execute(const jspan* const parent_span) {
  string   marker = s->info.args.get("marker"),
           shard = s->info.args.get("id"),
           err;
  unsigned shard_id;
...
void RGWOp_MDLog_Status::execute(const jspan* const parent_span)
{
...
void RGWOp_MDLog_Status::send_response(const jspan* const parent_span)
{
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s);
...
void RGWOp_BILog_Status::execute(const jspan* const parent_span)
{
...
void RGWOp_BILog_Status::send_response(const jspan* const parent_span)
{
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s);
...
void RGWOp_DATALog_Status::execute(const jspan* const parent_span)
{
  const auto source_zone = s->info.args.get("source-zone");
...
void RGWOp_DATALog_Status::send_response(const jspan* const parent_span)
{
  set_req_state_err(s, op_ret);
  dump_errno(s);
  end_header(s);
