...
void RGWRestUserPolicy::send_response(const jspan* const parent_span)
{
  if (op_ret) {
...
int RGWRestUserPolicy::verify_permission(const jspan* const parent_span)
{
...
void RGWPutUserPolicy::execute(const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWGetUserPolicy::execute(const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWListUserPolicies::execute(const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWDeleteUserPolicy::execute(const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
