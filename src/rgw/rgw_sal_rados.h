...
class RGWRadosUser : public RGWUser {
  ...
  public:
    ...
    int list_buckets(const std::string& marker, const std::string& end_marker,
				uint64_t max, bool need_stats, RGWBucketList& buckets, const jspan* const parent_span = nullptr);
...
class RGWRadosStore : public RGWStore {
  ...
  public:
    ...
    virtual std::unique_ptr<RGWObject> get_object(const rgw_obj_key& k) override;
    virtual int get_bucket(RGWUser* u, const rgw_bucket& b, std::unique_ptr<RGWBucket>* bucket, const jspan* const parent_span = nullptr) override;
    virtual int get_bucket(RGWUser* u, const RGWBucketInfo& i, std::unique_ptr<RGWBucket>* bucket) override;
    virtual int get_bucket(RGWUser* u, const std::string& tenant, const std::string&name, std::unique_ptr<RGWBucket>* bucket, const jspan* const parent_span = nullptr) override;
    virtual int create_bucket(RGWUser& u, const rgw_bucket& b,
			    ...
			    bool *existed,
			    req_info& req_info,
			    std::unique_ptr<RGWBucket>* bucket, const jspan* const parent_span = nullptr);
