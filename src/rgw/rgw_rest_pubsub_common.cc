...
void RGWPSCreateTopicOp::execute(const jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
void RGWPSListTopicsOp::execute(const jspan* const parent_span) {
  ups.emplace(store, s->owner.get_id());
...
void RGWPSGetTopicOp::execute(const jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
void RGWPSDeleteTopicOp::execute(const jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
void RGWPSCreateSubOp::execute(const jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
void RGWPSGetSubOp::execute(const jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
void RGWPSDeleteSubOp::execute(const jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
void RGWPSAckSubEventOp::execute(const jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
void RGWPSPullSubEventsOp::execute(const jspan* const parent_span) {
  op_ret = get_params();
  if (op_ret < 0) {
    return;
  }
...
int RGWPSCreateNotifOp::verify_permission(const jspan* const parent_span) {
  int ret = get_params();
  if (ret < 0) {
    return ret;
  }
...
int RGWPSDeleteNotifOp::verify_permission(const jspan* const parent_span) {
  int ret = get_params();
  if (ret < 0) {
    return ret;
  }
...
int RGWPSListNotifsOp::verify_permission(const jspan* const parent_span) {
  int ret = get_params();
  if (ret < 0) {
    return ret;
  }
