...
int RGWRestOIDCProvider::verify_permission(const jspan* const parent_span)
{
...
void RGWRestOIDCProvider::send_response(const jspan* const parent_span)
{
  if (op_ret) {
...
int RGWCreateOIDCProvider::verify_permission(const jspan* const parent_span)
{
  if (s->auth.identity->is_anonymous()) {
    return -EACCES;
  }
...
void RGWCreateOIDCProvider::execute(const jspan* const parent_span)
{
  op_ret = get_params();
  if (op_ret < 0) {
    return;
...
void RGWDeleteOIDCProvider::execute(const jspan* const parent_span)
{
...
void RGWGetOIDCProvider::execute(const jspan* const parent_span)
{
  RGWOIDCProvider provider(s->cct, store->getRados()->pctl, provider_arn, s->user->get_tenant());
  op_ret = provider.get();
...
int RGWListOIDCProviders::verify_permission(const jspan* const parent_span)
{
  if (s->auth.identity->is_anonymous()) {
    return -EACCES;
  }
...
void RGWListOIDCProviders::execute(const jspan* const parent_span)
{
