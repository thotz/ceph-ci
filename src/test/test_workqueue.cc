...
public:
    twq(time_t timeout, time_t suicide_timeout, ThreadPool *tp)
        : ThreadPool::WorkQueue<int>("test_wq", timeout, suicide_timeout, tp) {}
...
TEST(WorkQueue, change_timeout){
    ...
    ASSERT_EQ(2, wq.timeout_interval);
    ASSERT_EQ(20, wq.suicide_interval);

    // change the timeout and suicide and then check them
    wq.set_timeout(4);
    wq.set_suicide_timeout(40);
    ASSERT_EQ(4, wq.timeout_interval);
    ASSERT_EQ(40, wq.suicide_interval);
    tp.stop();   
}
