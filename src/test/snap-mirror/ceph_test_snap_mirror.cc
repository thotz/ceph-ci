...
int SnapReplicatorDispatcher::connect_to_cluster(snap_cluster_type_t sct)
{
  ...
  if (rv < 0) {
    std::clog << __func__ << ":" << __LINE__ << ": read cluster conf failed!\n";
    my_errno = -rv;
    return -1;
  }
  std::clog << __func__ << ":" << __LINE__ << ": read cluster conf completed\n";

  rv = ceph_conf_set(mnt, "keyring", keyring_path.c_str());
  if (rv < 0) {
    std::clog << __func__ << ":" << __LINE__ << ": ceph_conf_set(keyring, " << keyring_path << ") failed!\n";
    my_errno = -rv;
    return -1;
  }
  std::clog << __func__ << ":" << __LINE__ << ": keyring path set to " << keyring_path << "\n";
...
int main(int argc, const char **argv)
{
  if (argc < 10) {
    std::cerr << "Usage:\n\t";
    std::cerr << argv[0] << " <src conf> <dst conf> <src keyring path> <dst keyring path> <src dir> <src fs> <dst fs> <src auth id> <dst auth id> <old_snap_name> <new_snap_name>\n";
    ::exit(0);
  }
