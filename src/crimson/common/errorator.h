...
private:
  ...
  class _future<::crimson::errorated_future_marker<ValuesT...>>
    ...
    template <typename T>
    struct futurized_error {
      using ErrorT = T;
    };

    template <typename T>
    struct futurized_error<seastar::future<T>> {
      using ErrorT = T;
    };
  ...
  public:
    ...
    auto safe_then(ValueFuncT&& valfunc, ErrorVisitorT&& errfunc) {
      ...
      using return_errorator_t = make_errorator_t<
        ...
        std::decay_t<typename futurized_error<std::invoke_result_t<
		      ErrorVisitorT, AllowedErrors>>::ErrorT>...>;
