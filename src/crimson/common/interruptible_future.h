...
auto call_with_interruption(
  ...
  Func&& func, T&&... args)
{
  using futurator_t = seastar::futurize<Result>;
  ...
  if constexpr (sizeof...(T) == 1) {
    if constexpr (InterruptCond::template is_interruption_v<
	  std::tuple_element_t<0, std::tuple<T...>>>) {
      return futurator_t::make_exception_future(
	  std::get<0>(std::tuple(std::forward<T>(args)...)));
    }
    if constexpr (seastar::is_future<
	  std::tuple_element_t<0, std::tuple<T...>>>::value) {
      auto fut = std::get<0>(std::tuple(std::forward<T>(args)...));
      if (fut.failed()) {
	std::exception_ptr eptr = fut.get_exception();
	if (interrupt_condition->is_interruption(eptr)) {
	  return futurator_t::make_exception_future(std::move(eptr));
	}
	return internal::call_with_interruption_impl(
		  interrupt_condition,
		  std::forward<Func>(func),
		  seastar::futurize<decltype(fut)>::make_exception_future(
		    std::move(eptr)));
      }
      return internal::call_with_interruption_impl(
		interrupt_condition,
...
template <typename InterruptCond, typename Func, typename T,
	  typename Result = std::invoke_result_t<Func, T>>
decltype(auto) call_with_interruption_ret_err(
  InterruptCondRef<InterruptCond> interrupt_condition,
  Func&& func, T&& arg)
{
  bool set_int_cond = false;
  ...
  try {
    Result err = func(std::forward<T>(arg));
...
public:
  ...
  [[gnu::always_inline]]
  Result then_interruptible(Func&& func) {
    assert(interrupt_cond<InterruptCond>);
    return core_type::then(
      [func=std::move(func), interrupt_condition=interrupt_cond<InterruptCond>]
      (T... args) mutable {
      return call_with_interruption(
		interrupt_condition,
...
struct interruptible_errorator {
  ...
  make_ready_future(A&&... value) {
    return interruptible_future_detail<
      InterruptCond, typename Errorator::template future<T...>>(
	Errorator::template make_ready_future<T...>(std::forward<A>(value)...));
  }
...
template <typename InterruptCond, typename Errorator, typename... T>
...
public:
  ...
  static auto do_for_each(Iterator begin, Iterator end, AsyncAction&& action) {
    using Result = std::result_of_t<AsyncAction(decltype(*begin))>;
    assert(interrupt_cond<InterruptCond>);
    if constexpr (is_interruptible_future<Result>::value) {
  ...
  static auto repeat(AsyncAction&& action) {
    using Result = std::result_of_t<AsyncAction()>;
    assert(interrupt_cond<InterruptCond>);
    if constexpr (is_interruptible_future<Result>::value) {
      if constexpr (seastar::is_future<typename Result::core_type>::value) {
	return make_interruptible(
