...
void FrameAssembler::disasm_all_decompress(bufferlist segment_bls[]) const {
  ...
  for (size_t i = 0; i < m_descs.size(); i++) {
    ...
    if (!m_compression->rx->decompress(segment_bls[i], out)) {
      throw FrameError(fmt::format("Segment decompression failed"));
    }
    segment_bls[i].swap(out);
  }
}
