...
void RocksDBStore::compact_thread_entry()
{
  ...
  while (!compact_queue_stop) {
    if (!compact_queue.empty()) {
      auto& [s, e, remove, p] = compact_queue.front();
      ...
      if (s.empty() && e.empty()) {
        compact();  // FIXME minor: once here we can bypass all the following
                    // range compactions if any. Don't care for now..
      } else {
        bool has_range = false;
        if (!remove) {
          compact_range(s, e); // s & e already have prefix embedded
        } else {
          ...
          auto i = compact_queue.begin();
          for (++i; i != compact_queue.end();) {
            const auto& [s1, e1, r1, p1] = *i;
